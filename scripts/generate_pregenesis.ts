...
const PREGENESIS_OUTPUT = 'tmp/pre-genesis.json';
...
const generatePregenesis = (wallets: Wallets, genesisConfigFilepath: string, pregenesisOutputFilepath: string) => {
    ...
    fs.writeFileSync(pregenesisOutputFilepath, JSON.stringify(genesis, undefined, 2));
};
...
const main = async () => {
    ...
    generatePregenesis(wallets, GENESIS_CONFIG_FILE, PREGENESIS_OUTPUT);
